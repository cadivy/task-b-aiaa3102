...
> Implementation note: the complete repository was independently executed through an AI-assisted workflow at the user's request. The roles below are the recommended three-person ownership and verification plan for the group; they are not a claim that three named members already performed the recorded audit. The actual review provenance is documented in `adjudication_memo.csv` and `logs/chat.md`.
